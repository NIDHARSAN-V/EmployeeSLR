...
export default function HistoryPage() {
  ...
  const [kindFilter, setKindFilter] = useState<"all" | "ticket" | "asset">("all");

  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [tickets, assets] = await Promise.all([GetAllTickets(), GetAllAssets()]);
        const completed: HistoryItem[] = [
          ...tickets.filter((t) => t.status === "completed").map((t) => ({ ...t, _kind: "ticket" as const })),
          ...assets.filter((a) => a.status === "completed").map((a) => ({ ...a, _kind: "asset" as const })),
        ].sort((a, b) => {
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center">
        <p className="text-slate-500 font-mono text-sm animate-pulse">Loading history...</p>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 lg:p-8 font-sans">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-mono text-emerald-400 uppercase tracking-widest">History</span>
        </div>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-slate-100 tracking-tight">Completed Work</h1>
            <p className="text-sm text-slate-500 mt-1 font-mono">
              <span className="text-amber-400">{ticketCount}</span> tickets ·{" "}
              <span className="text-cyan-400">{assetCount}</span> assets resolved
            </p>
          </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-6">
        ...
        <p className="text-xs font-mono text-slate-600 whitespace-nowrap ml-auto">
          {filtered.length === items.length ? `${items.length} items` : `${filtered.length} of ${items.length}`}
        </p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-xs text-slate-600 font-mono mt-1">Completed tickets and assets will appear here</p>
        </div>
      ) : (
        <div className="bg-slate-900 border border-slate-700/60 rounded-lg overflow-hidden">
          <div className="divide-y divide-slate-800/60">
            {filtered.map((item) => (
              <HistoryRow key={item.refId} item={item} />
            ))}
          </div>
        </div>
      )}
    ...
    <div className="px-4 py-3 hover:bg-slate-800/40 transition-colors">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] font-mono text-slate-600 mt-0.5">#{item.refId}</p>
        </div>
        ...
        {/* Completed at */}
        <p className="shrink-0 text-xs font-mono text-slate-500 hidden sm:block">{completedDate}</p>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-slate-800/60 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { label: "Raised By", value: item.raised_by },
            { label: "Accepted By", value: item.accepted_by ?? "—" },
            { label: "Completed By", value: item.completed_by ?? "—" },
            { label: "Created", value: createdDate },
            {
              label: "Accepted At",
              value: item.acceptedAt
                ? new Date(item.acceptedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
                : "—",
            },
            { label: "Completed At", value: completedDate },
            {
              label: "Accept Due",
              value: item.acceptDueAt
                ? new Date(item.acceptDueAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })
                : "—",
            },
            {
              label: "Complete Due",
              value: item.completeDueAt
                ? new Date(item.completeDueAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })
                : "—",
            },
          ].map(({ label, value }) => (
            <div key={label}>
