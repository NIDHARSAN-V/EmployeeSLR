...
const STAGES: { key: Asset["status"]; label: string; color: string; bar: string }[] = [
  { key: "pending", label: "Pending", color: "text-amber-400", bar: "bg-amber-400" },
  { key: "accepted", label: "Accepted", color: "text-sky-400", bar: "bg-sky-400" },
    ...
    <div className="bg-slate-900 border border-slate-700/60 rounded-lg p-4 mb-6">
      ...
      <div className="flex items-center gap-0 mb-3 overflow-hidden rounded-full h-2">
        {counts.map((s, i) => (
          <div
            key={s.key}
            className={`h-full transition-all ${s.bar} ${i === 0 ? "" : "border-l border-slate-950"}`}
