...
async function http(baseUrl, path, method = "GET", body) {
...
async function pingServer(baseUrl) {
    ...
    if (!res)
        throw new Error(`❌ Cannot reach server at ${baseUrl}. Start backend first.`);
}
...
async function seedDemo(baseUrl) {
    ...
    try {
        ...
        async function registerAndLogin(userName, email, role) {
            await http(BASE_URL, "/auth/register", "POST", {
                userName,
                email,
                password: PASSWORD,
                role,
            });
            const login = await http(BASE_URL, "/auth/login", "POST", {
                email,
                password: PASSWORD,
            });
        ...
        for (let i = 0; i < 18; i++) {
            ...
            const created = await http(BASE_URL, "/tickets", "POST", {
        ...
        for (let i = 0; i < 12; i++) {
            ...
            const created = await http(BASE_URL, "/assets", "POST", {
                request_type: reqType,
                raised_by: raisedBy,
            });
        ...
        const adminDeadline = await http(BASE_URL, `/notifications/deadline/${adminId}`, "GET");
        logs.push("⛔ Fetching OVERDUE notifications (and writing SLA breaches)...");
        const adminEnded = await http(BASE_URL, `/notifications/ended/${adminId}`, "GET");
        // -----------------------------
        // Prepare cheat sheet
        // -----------------------------
