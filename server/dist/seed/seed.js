...
async function pingServer() {
    ...
    if (!res)
        throw new Error(`❌ Cannot reach server at ${BASE_URL}. Start backend first.`);
...
async function registerAndLogin(userName, email, role) {
    ...
    const login = await http("/auth/login", "POST", { email, password: PASSWORD });
...
async function seed() {
    ...
    const adminDeadline = await http(`/notifications/deadline/${adminId}`, "GET");
    console.log("\n--- ADMIN NEAR DEADLINE ---");
    console.log(JSON.stringify(adminDeadline, null, 2));
    const resolver1Deadline = await http(`/notifications/deadline/${resolver1Id}`, "GET");
    ...
    const adminEnded = await http(`/notifications/ended/${adminId}`, "GET");
    console.log("\n--- ADMIN OVERDUE ---");
    console.log(JSON.stringify(adminEnded, null, 2));
    const resolver1Ended = await http(`/notifications/ended/${resolver1Id}`, "GET");
    console.log("\n--- RESOLVER1 OVERDUE ---");
